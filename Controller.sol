...
contract ControllerHub is AccessControlEnumerable, Ownable {
    ...
    mapping  (address => bool) public strategieListExist;
    mapping  (address => uint256) public strategieListNumber; 
    address[] public strategieListPrioritySort; //from small to big

    address[] public rewardToken;
    mapping  (address => bool) public rewardTokenExist;
    ...
    event VaultChanged(address);
    event AddRewardToken(address);
    event PauseChanged(bool);
    event LockChanged(bool);
    event BetaChanged(bool);

    event AddStrategy(address);

    event Emergencywithdraw(address,uint256,uint256);
    event EmergencyTokensGetStuck( address , address, uint256 );
    
    event Earn(address );
    event GovEarn(uint256 );
    event GovEarnbyStrategyAddress(address );
    event WithdrawAll(address );
    event WithdrawLp(address ,uint256 );
    event GovWithdraw(uint256 , address ,uint256 );
    event GovWithdrawStrategyAddress(address, address, uint256);

    event GovWithdrawSingleTokenForRebalance(address ,uint256 );
    event WithdrawReward(address, address );

    constructor(){
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());//owner privilege, need remove after setting all
    ...
    function harvestSingleTokenReward(address _singleToken) onlyWhiteList public{
            ...
            for(uint i = 0;i < strategieList.length; i++){
                ...
                _strategy.withdrawReward();
            }        
    }
    
	//stategy callback  
    function CollectBlockReward(uint256 _amount, address _rewardToken) public onlyWhiteList  {
        ...
        for(uint i=0; i<strategieList.length; i++){
           ...
           _strategy = IStrategy(strategieList[i]);            
            ...
            for(uint ii=0; ii<rewardToken.length; ii++){
                _rewardTokens[ii] +=  strategieListEarned[strategieList[i]][ii]/tokenStrategyNeed.length;//average distribute
            }
        }
    }
    
    //set vault address, only once
    function setVault(address _vault)external onlyGovernance{
		require(vault==address(0));
        vault=_vault;
        _setupRole(INTERFACE_ROLE, _vault);
    ...
    function addRewardToken(address _rewardToken) public onlyGovernance{
        require(_rewardToken!=address(0x0),"address error");
    ...
    //paused only limited to call earn()
    ...
    function addStrategieList(address _strategy) Unlocked external onlyGovernance{
        ...
        //check strategy contains the necessary interfaces
        ...
        strategieListPrioritySort.push(_strategy);
         _setupRole(INTERFACE_ROLE, _strategy);    
    ...
    function setstrategieListPrioritySort(address[] memory prioritys) external onlyGovernance{
    ...
    function govEarnbyStrategyAddress(address stategyAddress) public onlyGovernance {
        ...
        emit GovEarnbyStrategyAddress(stategyAddress );
    ...
    function withdrawLp(address _singleToken,uint256 _amount) public onlyWhiteList {
        ...
        emit WithdrawLp( _singleToken, _amount);
    }
    
    
    //rebalance tokens,check oversize user quota
    function govWithdrawSingleTokenForRebalance(address _singleToken,uint256 value) public onlyGovernance{ 
        ...
        if(available>totalDeposit){
            ...
            }else if(value>available-totalDeposit){
                value=available-totalDeposit;
            }
            IERC20(_singleToken).safeTransferFrom(vault,msg.sender,value);
        }   

        emit GovWithdrawSingleTokenForRebalance( _singleToken,value ) ;    
    ...
    function govWithdraw(uint256 _sid, address _singleToken,uint256 amount) public onlyGovernance{
        ...
        emit GovWithdraw( _sid, _singleToken, amount);
    }
    ...
    function govWithdrawStrategyAddress(address stategyAddress, address _singleToken,uint256 amount) public onlyGovernance{
        ...
        emit GovWithdrawStrategyAddress(stategyAddress, _singleToken, amount);
    }
    ...
    function withdrawReward(address _rewardToken,address receiver) public onlyGovernance returns (bool){
        ...
        emit WithdrawReward( _rewardToken, receiver);
        return true;
    }


    bool public afterBeta;
    
    //set contract status
    function setAfterBeta() public onlyEmergencyWithdraw {
        afterBeta=true;
        emit BetaChanged( afterBeta );
    }
    ...
    function inCaseTokensGetStuck(address withdrawaddress,address _token,uint _amount)  public onlyEmergencyWithdraw {
        ...
        emit EmergencyTokensGetStuck( withdrawaddress, _token, _amount);
    }    
