...
contract VaultPool is AccessControlEnumerable, Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo{
        ...
        uint256[10] accRewardShare;
        uint256 totalAmount;
        uint256[10] profit;
        uint256[10] accRewardPerShare; 
        uint256[10] govAccRewardPerShare;
        uint256[10] lastRewardBlockProflt;
    ...
    mapping  (address => bool) public poolInfoExist;
    mapping  (address => uint256 )public poolInfoNumber;
    ...
    bytes32 public constant GOVERNANCE_ROLE = keccak256("GOVERNANCE_ROLE");//need set multi-sign address
    bytes32 public constant WITHDRAW_ROLE = keccak256("WITHDRAW_ROLE");
    bytes32 public constant INTERFACE_ROLE = keccak256("INTERFACE_ROLE");      
    
    constructor(address _controller){
        require(_controller!=address(0x0),"address error");
    ...
    function setLocked(bool _locked)external onlyGovernance{
        locked=_locked;
    ...
    function setController(address _controller)internal {
        controller=_controller;
         _setupRole(INTERFACE_ROLE, _controller);   
    }
    ...
    function rewardTokens() public view returns(address[] memory rewardTokenArr){
        ...
        if(rewardTokenArr.length>10){
            //only return maximize 10 tokens
            address[] memory rewardTokenArr1=new address[](10);
            for(uint256 i = 0 ; i < 10; i ++ ){
                rewardTokenArr1[i]=rewardTokenArr[i];
            }
    ...
    function getSingleTotalReward(address _singleToken) public view returns (uint256[] memory reward) {
    ...
    function getPoolInfoDetail(uint256 _pid,uint256 _rewardid) public view returns (uint256 ,uint256,uint256,uint256,uint256) {
        ...
        return (pool.accRewardShare[_rewardid],pool.profit[_rewardid],pool.accRewardPerShare[_rewardid],pool.govAccRewardPerShare[_rewardid],pool.lastRewardBlockProflt[_rewardid]);
    }
    ...
    function updatePool(uint256 _pid) public notPause allowed{ 
        ...
        for(uint i=0; i<rewardTokens().length; i++){
            ...
            uint256 increment = tokenTotalReward.sub(pool.profit[i]);
            govTotalProfit[i] = govTotalProfit[i].add(increment);
          
            
            pool.accRewardShare[i] = pool.accRewardShare[i].add(increment.mul(1e12).div(pool.totalAmount));
            pool.govAccRewardPerShare[i] = pool.govAccRewardPerShare[i].add(increment.mul(1e12).div(pool.totalAmount));   
            
            pool.lastRewardBlockProflt[i]=increment.mul(1e12).div(pool.totalAmount).div(block.number.sub(pool.lastRewardBlock));
    ...
    function depositWithPid(uint256 _pid, uint256 _amount) public notPause allowed {
        require(_amount >= 0,"deposit: not good");
        PoolInfo storage pool = poolInfo[_pid];

        require( address(pool.token) != address(0x0),"pool is not exist");
        ...
        if (_amount > 0){
			...
			require(_amount == afterToken.sub(beforeToken));            
            if(_amount > 0){
    ...
    function earn(address token) public notPause allowed{
    ...
    function withdrawWithPid(uint256 _pid, uint256 _amount) public notPause allowed{
    ...
    function emergencyWithdraw(uint256 _pid) public notPause  allowed{ 
        PoolInfo storage pool = poolInfo[_pid];
        require( address(pool.token) != address(0x0),"pool is not exist");
