...
contract WrapperToken is ERC20PresetMinterPauser , Ownable ,ReentrancyGuard{
    uint8 tokenDecimals;
...
contract PoolWrapperToken is WrapperToken{
    using SafeMath for uint256;
    ...
    //intend to avoid flash load attack
    uint256 public frozenStakingTime = 3;

    address public defiFundAddress;
    ...
    event WithdrawRewardToFundAddress(address ,uint256 );
    event SetDefiFundAddress(address );
    event SetFrozenStakingTime(uint256 );
    event WithdrawEmergency(address ,address );
    ...
    constructor( 
    ...
    ) WrapperToken(name, symbol,_decimals)  {
        require(singleToken_!=address(0x0),"address error");
        ...
        defiFundAddress=_msgSender();

        singleToken.safeApprove(vaultAddress,0);  
    ...
    function stake(uint256 amount) 
        public  onlyNotBlackList nonReentrant
    {
        if(amount==0){
            return;
        }
        ...
        public  onlyNotBlackList nonReentrant
    {
        ...
        require(block.timestamp >= unfrozenStakeTime(msg.sender), "wrapperTokenPool: Cannot withdraw during freezing");
    ...
    function withdrawRewardToFundAddress(address rewardTokenAddress,uint256 amount) external virtual  {
        ...
        require(defiFundAddress != address(0), 'wapperTokenPool: Zero address or self');
		require(rewardTokenAddress!=address(singleToken),"this is rewardToken!");		
        IERC20(rewardTokenAddress).transfer(defiFundAddress,amount);

        emit WithdrawRewardToFundAddress(rewardTokenAddress, amount);
    }    


    function setDefiFundAddress(address defiFundAddress_) external virtual  {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),  "wapperTokenPool: must have DEFAULT_ADMIN_ROLE");
        require(defiFundAddress_ != address(0) && defiFundAddress_ != address(this), 'wapperTokenPool: Zero address or self');
        defiFundAddress = defiFundAddress_;

        emit SetDefiFundAddress( defiFundAddress_);
    }
    ...
    function setFrozenStakingTime(uint256 frozenStakingTime_) external virtual  {
        ...
        emit SetFrozenStakingTime( frozenStakingTime_ );
    }
    ...
    function withdrawEmergency(address tokenaddress,address to) public {
        ...
        emit WithdrawEmergency(tokenaddress, to);
    }
